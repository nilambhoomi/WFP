<%@ Page Title="" Language="C#" MasterPageFile="~/common.master" AutoEventWireup="true" CodeFile="IEReport.aspx.cs" Inherits="IEReport" %>
...
.gvLatestVisit a /** FOR THE PAGING ICONS  **/
{
...
.gvLatestVisit a:hover /** FOR THE PAGING ICONS  HOVER STYLES**/
{
    background-color: #000;
    color: #fff;
}
.gvLatestVisit span /** FOR THE PAGING ICONS CURRENT PAGE INDICATOR **/
{
...
body
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            $.ajax({
                type: "POST",
                url: "IEReport.aspx/GetLatestVisit",
        ...
        function OnSuccess(response) {
            ...
            var row = $("[id*=gvLatestVisit] tr:last-child").clone(true);
            $("[id*=gvLatestVisit] tr").not($("[id*=gvLatestVisit] tr:first-child")).remove();
            $.each(customers, function () {
                ...
                $("td", row).eq(3).html($(this).find("DOE").html());

                $("[id*=gvLatestVisit]").append(row);
                row = $("[id*=gvLatestVisit] tr:last-child").clone(true);
            });
...
<%--                 <asp:Button ID="Search" runat="server" CssClass="btn btn-danger" Text="Search"/>--%>
        ...
        <asp:GridView ID="gvLatestVisit" runat="server" CssClass="gvLatestVisit" PagerStyle-CssClass="pager" AutoGenerateColumns="false"
